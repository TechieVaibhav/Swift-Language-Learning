...
@propertyWrapper struct MyIntWraper{
    ...
    var wrappedValue: Int{
        set {
            _storeValue = newValue * 10
        } get {
            return _storeValue
        }
    }
    init(passvalue : Int) {
        self._storeValue = passvalue
    }
}
...
class TestWrapper{
    ...
    @MyIntWraper var bigNumber : Int = 0

    init(bigNumber : MyIntWraper) {
        self.bigNumber = bigNumber.wrappedValue
    }

}
let wrapperObj = TestWrapper(bigNumber: MyIntWraper(passvalue: 10))
print(wrapperObj.bigNumber)
